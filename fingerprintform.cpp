...
namespace {
constexpr int kFingerFeatureBytes = 512;
constexpr short kMinimumFingerprintMatchScore = 50;
}
...
FingerFeatureData FingerPrintForm::getRecognizeResult(QByteArray fingerFeature){
    ...
    if (fingerFeature.size() != kFingerFeatureBytes) {
        qWarning() << "Invalid fingerprint feature size:" << fingerFeature.size();
        return resultData;
    }
    ...
    for(int i = 0;i < mFeatureDataList.size();++i){
        if (mFeatureDataList[i].featureData.size() != kFingerFeatureBytes) {
            qWarning() << "Skipping invalid fingerprint feature for user:" << mFeatureDataList[i].userNo;
            continue;
        }
        ...
        if(tmpScore >= kMinimumFingerprintMatchScore && tmpScore > bestScore){
            bestScore = tmpScore;
