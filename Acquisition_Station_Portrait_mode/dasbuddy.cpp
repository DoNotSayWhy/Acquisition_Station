  ...
  ui(new Ui::DASBuddy)
{
    ...
    // Keep the idle emblem centered by letting the four surrounding spacers
    // expand symmetrically with the actual list-card size.
    // A small left inset shifts the idle emblem slightly to the right while
    // preserving its vertical centering in every responsive port card.
    ui->gridLayout->setContentsMargins(20, 0, 0, 0);
    ...
    ui->horizontalSpacer_4->changeSize(0, 0, QSizePolicy::Expanding, QSizePolicy::Minimum);
    ui->verticalSpacer->changeSize(0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding);
    ui->verticalSpacer_2->changeSize(0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding);
...
void DASBuddy::setCurrentStatus(int status){
    ...
    // A palette-only reset can leave the cyan collecting artwork visible after
    // a recorder is removed. Force an idle repaint on the card itself.
    QPalette p = palette();
    if (status == 0) {
        this->setStyleSheet("background-color: rgb(3, 37, 70);");
        p.setBrush(this->backgroundRole(), QBrush(QColor(3, 37, 70)));
    } else {
        this->setStyleSheet(QString());
        p.setBrush(this->backgroundRole(), QBrush(QPixmap(imagePath).scaled(
...
void DASBuddy::on_devicePairButton_clicked()
{
      ...
      // connect(okButton,&QPushButton::clicked,this,&DASBuddy::writeHsParamIni);
       connect(okButton,&QPushButton::clicked,this,[=,&dialog](){
           ...
           QString _admName = "888888";
           QString _admPwd = "888888";

           if(_admNo == _admName && _pwdNo == _admPwd){

           }else{

               //QMessageBox::information(nullptr, "校验失败", "用户或密码错误,请检查!");
               //return;
           }
