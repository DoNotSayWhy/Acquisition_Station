...
void WsPairDeviceForm::checkConfigPortInfo(){
    pConfig = new Config();
    deviceCount = qBound(1, deviceCount, 999);
    const int oldCount = pConfig->Get("portInfo", "port_num").toInt();
    pConfig->Set("portInfo", "port_num", deviceCount);

    // A physical USB path may only occur once. Repair stale duplicates before
    // MainForm reads the mapping at the next restart.
    QSet<QString> pairedUsbPorts;
    for (int i = 0; i < deviceCount; ++i) {
        const QString key = "port" + QString::number(i);
        const QString value = pConfig->Get("portInfo", key).toString().trimmed();
        if (value.isEmpty() || value == "-1" || pairedUsbPorts.contains(value)) {
            pConfig->Set("portInfo", key, "-1");
        } else {
            pairedUsbPorts.insert(value);
        }
    }
    for (int i = deviceCount; i < oldCount; ++i) {
        pConfig->Set("portInfo", "port" + QString::number(i), "-1");
    }
    pConfig->Sync();
}
...
void WsPairDeviceForm::pushButtonPairDevice(QString portNum){
    ...
    if(isOk){
        int realNum = num - 1;
        if (realNum < 0 || realNum >= deviceForm.size() || !deviceForm.at(realNum)) {
            QMessageBox::warning(this, "提示", "端口编号无效，请重新打开端口匹配页面。");
            return;
        }
        int originStatus = deviceForm[realNum]->getPairStatus();
        if(originStatus == 0){
            // Move the recorder instead of keeping two logical ports bound to
            // it, which previously sent collection events to the wrong card.
            for (int i = 0; i < deviceForm.size(); ++i) {
                if (i == realNum || !deviceForm.at(i)) {
                    continue;
                }
                const QString key = "port" + QString::number(i);
                if (pConfig->Get("portInfo", key).toString().trimmed() == currentPairPort) {
                    pConfig->Set("portInfo", key, "-1");
                    deviceForm[i]->setPairStatus(0);
                }
            }
            ...
            pConfig->Sync();
            QMessageBox::information(this, "提示", "端口匹配已保存，重启软件后生效。");
        } else {
