...
void FaceFormUtility::on_pushButtoncencle_clicked()
{
    ...
    if (enrollThread && enrollThread->isRunning()) {
        enrollThread->requestInterruption();
        enrollThread->wait();
...
void FaceFormUtility::enroll_face() {
    ...
    ui->labelfacestatus->setText("正在准备人脸录入…");

    if (!enrollThread || !enrollThread->isRunning()) {
        // 创建并启动人脸录入线程
        enrollThread = new FaceEnrollThread(src.clone(), this);
        connect(enrollThread, &FaceEnrollThread::faceEnrollNewFrame, this, &FaceFormUtility::onfaceEnrollFrame, Qt::QueuedConnection);
        connect(enrollThread, &FaceEnrollThread::updateImage, this, &FaceFormUtility::updateImage, Qt::QueuedConnection);
        connect(enrollThread, &FaceEnrollThread::enrollmentStatus, this, &FaceFormUtility::updateLabelFaceStatusERROR, Qt::QueuedConnection);
        connect(enrollThread, &FaceEnrollThread::finished, this, [this]() {
            if (enrollThread) {
                enrollThread->deleteLater();
                enrollThread = nullptr;
            }
        });
        connect(enrollThread, &FaceEnrollThread::enrollmentFailed, this, &FaceFormUtility::updateLabelFaceStatusERROR, Qt::QueuedConnection);
...
void FaceFormUtility::updateLabelFaceStatusERROR(const QString &message) {
    ui->labelfacestatus->setVisible(true);
    ui->labelfacestatus->setText(message);
}
